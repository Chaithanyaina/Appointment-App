...
const Register = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            toast.error(err.response?.data?.error?.message || 'Failed to register.', { id: loadingToast });
        } finally {
            setLoading(false);
        }
    };
        ...
        <motion.div 
            ...
            <div className="w-full max-w-md p-8 space-y-8 bg-gray-900 rounded-2xl shadow-2xl border border-gray-800">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    <div>
                        <input id="name" type="text" value={name} onChange={(e) => setName(e.target.value)} required className="w-full px-4 py-3 bg-gray-800 border border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all" placeholder="Full Name"/>
                    </div>
                    <div>
